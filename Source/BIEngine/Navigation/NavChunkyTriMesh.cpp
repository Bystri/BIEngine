...
static void subdivide(BoundsItem* items, int nitems, int imin, int imax, int trisPerChunk, int& curNode, NavChunkyTriMeshNode* nodes, const int maxNodes, int& curTri, int* outTris, const int* inTris)
{
   int inum = imax - imin;
   int icur = curNode;
   ...
   NavChunkyTriMeshNode& node = nodes[curNode++];
...
bool NavCreateChunkyTriMesh(const float* verts, const int* tris, int ntris, int trisPerChunk, NavChunkyTriMesh* cm)
{
   ...
   cm->nodes = new NavChunkyTriMeshNode[nchunks * 4];
   if (!cm->nodes) {
      return false;
   }
   ...
   for (int i = 0; i < cm->nnodes; ++i) {
      NavChunkyTriMeshNode& node = cm->nodes[i];
...
int NavGetChunksOverlappingRect(const NavChunkyTriMesh* cm, float bmin[2], float bmax[2], int* ids, const int maxIds)
{
   ...
   int n = 0;
   while (i < cm->nnodes) {
      const NavChunkyTriMeshNode* node = &cm->nodes[i];
