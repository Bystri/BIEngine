...
void SelectedActorOutliner::OnRender(BIEngine::Scene* const pScene, BIEngine::RenderItemsStorage* const pStorage)
{
   ...
   for (const auto& ritem : opaqueItems) {
      ...
      BIEngine::RenderCommand renderCommandStencil(ritem.pMesh.get(), m_pEmptyColorSp);
      ...
      BIEngine::RenderCommand renderCommandOutline(ritem.pMesh.get(), m_pSolidColorSp);
